...
OUT_DIR="experiments/results/local_docker_mock_vlm"
mkdir -p "$OUT_DIR"
...
p = Path("experiments/results/local_docker_mock_vlm/performance_summary.json")
payload = {
    "environment_name": "local_docker_mock_vlm",
    "status": "NOT_MEASURED",
    "run_location": "local",
    ...
    "note": "Local Docker runtime unavailable or unresponsive; mock container run not measured.",
}
...
docker run --rm \
  ...
  --environment-name local_docker_mock_vlm \
